...
datalist <- prepare_ddm_list(data = animacysize_clean, id_name = "nomem_encr", n_conditions = 2)
...
## 4.1 Fit ----
ddm_animacysize_fit <- run.jags(method = "parallel",
                            model = model_2con,
                            monitor = parameters,
                            data = datalist,
...
mcmc_animacysize <- as.matrix(as.mcmc.list(ddm_animacysize_fit), chains = F) |>
  as_tibble()

# Intermediate save in case the session crashes
save(mcmc_animacysize, ddm_animacysize_fit, file = "1_data/2_IntermediateData/DDM_animacysize_fit.RData")
...
animacysize_traces    <- extract_traces_2con(mcmc_animacysize, chains = nChains, iterations = nIterations)
animacysize_param_est <- extract_ddm_estimates_2con(mcmc = mcmc_animacysize, task_prefix = "as_", id_matches = animacysize_id_matches)

## 5.2 Simulation-based model fit assessment ----
animacysize_sim_fit <- animacysize_param_est %>%
  mutate(
    responses = pmap(., function(nomem_encr, as_v1, as_v2, as_a1, as_a2, as_t1, as_t2) {
      
      bind_rows(
        # Condition 1
        RWiener::rwiener(n=500, alpha=as_a1, tau=as_t1, beta=0.5, delta=as_v1) |>
          as_tibble() |>
          mutate(
            condition = 1
          ),
        # Condition 2
        RWiener::rwiener(n=500, alpha=as_a2, tau=as_t2, beta=0.5, delta=as_v2) |>
          as_tibble() |>
          mutate(
            condition = 2
          )
...
save(mcmc_animacysize, ddm_animacysize_fit, animacysize_traces, animacysize_param_est, animacysize_sim_fit, file = "1_data/2_IntermediateData/DDM_animacysize_fit.RData")
...
ddm_animacysize_refit <- run.jags(method = "parallel",
                              model = model_2con,
                              monitor = parameters,
                              data = datalist,
                              inits = initfunction_2con,
...
rhat_animacysize <- ddm_animacysize_refit$psrf$psrf |>
  unlist() |>
