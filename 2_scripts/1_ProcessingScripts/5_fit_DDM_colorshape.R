...
datalist <- prepare_ddm_list(data = colorshape_clean, id_name = "nomem_encr", n_conditions = 2)
...
## 4.1 Fit ----
ddm_colorshape_fit <- run.jags(method = "parallel",
                            model = model_2con,
                            monitor = parameters,
                            data = datalist,
...
mcmc_colorshape <- as.matrix(as.mcmc.list(ddm_colorshape_fit), chains = F) |>
  as_tibble()

# Intermediate save in case the session crashes
save(mcmc_colorshape, ddm_colorshape_fit, file = "1_data/2_IntermediateData/DDM_colorshape_fit.RData")
...
colorshape_traces    <- extract_traces_2con(mcmc_colorshape, chains = nChains, iterations = nIterations)
colorshape_param_est <- extract_ddm_estimates_2con(mcmc = mcmc_colorshape, task_prefix = "cs_", id_matches = colorshape_id_matches)

## 5.2 Simulation-based model fit assessment ----
colorshape_sim_fit <- colorshape_param_est %>%
  mutate(
    responses = pmap(., function(nomem_encr, cs_v1, cs_v2, cs_a1, cs_a2, cs_t1, cs_t2) {
      
      bind_rows(
        # Condition 1
        RWiener::rwiener(n=500, alpha=cs_a1, tau=cs_t1, beta=0.5, delta=cs_v1) |>
          as_tibble() |>
          mutate(
            condition = 1
          ),
        # Condition 2
        RWiener::rwiener(n=500, alpha=cs_a2, tau=cs_t2, beta=0.5, delta=cs_v2) |>
          as_tibble() |>
          mutate(
            condition = 2
          )
...
save(mcmc_colorshape, ddm_colorshape_fit, colorshape_traces, colorshape_param_est, colorshape_sim_fit, file = "1_data/2_IntermediateData/DDM_colorshape_fit.RData")
...
ddm_colorshape_refit <- run.jags(method = "parallel",
                              model = model_2con,
                              monitor = parameters,
                              data = datalist,
                              inits = initfunction_2con,
...
rhat_colorshape <- ddm_colorshape_refit$psrf$psrf |>
  unlist() |>
...
save(mcmc_colorshape, ddm_colorshape_fit, colorshape_traces, colorshape_param_est, colorshape_sim_fit, rhat_colorshape, file = "1_data/2_IntermediateData/DDM_colorshape_fit.RData")
