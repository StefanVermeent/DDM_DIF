...
data_clean <- data_clean |> 
  mutate(
    across(matches("t(1|2)_c"), ~scale(.) |> as.numeric()),
...
model <-   
  '
  # Factor loadings
  A =~ NA*fl_t1_c + fl_t2_c + si_t1_c + si_t2_c + cs_t1_c + cs_t2_c + gl_t1_c + gl_t2_c + as_t1_c + as_t2_c
...
manVars <- colnames(data_clean |> select(matches("_t(1|2)_c$")))  
nv <- length(manVars)
manVars
...
anchorOut_t <- data.frame(Name=paste0("Indicator",1:nv), 
                          X2=anchorTest_t$diffLL[seq(2,(nv*2),2)],
                          df=anchorTest_t$diffdf[seq(2,(nv*2),2)],
                          p=anchorTest_t$p[seq(2,(nv*2),2)]) %>% 
  arrange(X2)
