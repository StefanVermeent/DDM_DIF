...
data_clean <- data_clean |> 
  mutate(
    across(matches("t(1|2)"), ~scale(.) |> as.numeric(), .names = "{.col}_l"),
...
model <-   
  '
  # Factor loadings
  A =~ NA*fl_t1_l + fl_t2_l + si_t1_l + si_t2_l + cs_t1_l + cs_t2_l + gl_t1_l + gl_t2_l + as_t1_l + as_t2_l
...
config_age_t <- lavaan::cfa(
  model = model, 
  data = data_clean_conf, 
  group = "age_cat",
  missing = "ml")

config_age_t_sum <- summary(config_age_t, fit.measures = TRUE, standardized = TRUE)
config_age_t_fitstats <- config_age_t_sum$fit[c('chisq', 'df', 'pvalue', 'cfi', 'rmsea', 'rmsea.ci.lower', 'rmsea.ci.upper')]

## 2.3 Configural invariance for education ----

config_edu_t <- lavaan::cfa(
  model = model, 
  data = data_clean_conf, 
  group = "edu_cat",
  missing = "ml")

config_edu_t_sum <- summary(config_edu_t, fit.measures = TRUE, standardized = TRUE)
config_edu_t_fitstats <- config_edu_t_sum$fit[c('chisq', 'df', 'pvalue', 'cfi', 'rmsea', 'rmsea.ci.lower', 'rmsea.ci.upper')]

## 2.4 Configural invariance for urbanicity ----

config_urb_t <- lavaan::cfa(
  model = model, 
  data = data_clean_conf, 
  group = "urb_cat",
  missing = "ml")

config_urb_t_sum <- summary(config_urb_t, fit.measures = TRUE, standardized = TRUE)
config_urb_t_fitstats <- config_urb_t_sum$fit[c('chisq', 'df', 'pvalue', 'cfi', 'rmsea', 'rmsea.ci.lower', 'rmsea.ci.upper')]

## 2.5 Configural invariance for threat

config_thr_t <- lavaan::cfa(
  model = model, 
  data = data_clean_conf, 
  group = "child_thr_cat",
  missing = "ml")

config_thr_t_sum <- summary(config_thr_t, fit.measures = TRUE, standardized = TRUE)
config_thr_t_fitstats <- config_thr_t_sum$fit[c('chisq', 'df', 'pvalue', 'cfi', 'rmsea', 'rmsea.ci.lower', 'rmsea.ci.upper')]

## 2.5 Configural invariance for threat

config_dep_t <- lavaan::cfa(
  model = model, 
  data = data_clean_conf, 
  group = "child_dep_cat",
  missing = "ml")

config_dep_t_sum <- summary(config_dep_t, fit.measures = TRUE, standardized = TRUE)
config_dep_t_fitstats <- config_dep_t_sum$fit[c('chisq', 'df', 'pvalue', 'cfi', 'rmsea', 'rmsea.ci.lower', 'rmsea.ci.upper')]


save(config_age_t_fitstats, config_edu_t_fitstats, config_urb_t_fitstats, config_thr_t_fitstats, config_dep_t_fitstats,
     file = "3_output/Results/1_SEM/MI_config_t.RData")
...
manVars <- colnames(data_clean |> select(matches("_t(1|2)_l$")))  
nv <- length(manVars)
manVars
...
modConfig_t <- mxModel(model="Configural", 
...
modScalar_t <- mxModel(model="Scalar", 
                       matT, matT0, matB1, matB2, matB3, matB4, matB5,
                       matL, matL0, matC1, matC2, matC3, matC4, matC5,
                       matE, matE0, matD1, matD2, matD3, matD4, matD5,
                       matP, matP0, matH1, matH2, matH3, matH4, matH5,
...
fitScalar_t <- mxRun(modScalar_t)
summary(fitScalar_t)


### 3.3.4 Compare fit of unconstrained model with constrained model ----

omn_miTest_t <- mxCompare(fitConfig_t, fitScalar_t)
omn_miTest_t[2,c(7,8,9)]
omn_miTest_t$p[2] < 0.001

save(fitConfig_t, fitScalar_t, omn_miTest_t, file = "3_output/Results/2_MNLFA/MNLFA_fit_t.RData")
...
for (i in 1:nv) {
  ...
  matC5 <- mxMatrix(type="Full", nrow=nv, ncol=1,
                    ...
                    byrow=TRUE,
                    name="matC5")
  modAbo_t <- mxModel(model=paste0("All_but_", i), 
                      matT, matT0, matB1, matB2, matB3, matB4, matB5,
                      matL, matL0, matC1, matC2, matC3, matC4, matC5,
                      matE, matE0, matD1, matD2, matD3, matD4, matD5,
                      matP, matP0, matH1, matH2, matH3, matH4, matH5,
  ...
  fitAbo_t[[i]] <- mxRun(modAbo_t) 
}

### 3.4.2 Compare constrained model with all unconstrained models ----

anchorTest_t <- mxCompare(fitAbo_t, fitScalar_t)
anchorOut_t <- data.frame(Name=paste0("Indicator",1:10), 
                          X2=anchorTest_t$diffLL[seq(2,20,2)],
                          df=anchorTest_t$diffdf[seq(2,20,2)],
                          p=anchorTest_t$p[seq(2,20,2)]) %>% 
  arrange(X2)


### 3.4.3 Save anchors in object ----

anchors_t <- head(anchorOut_t,2) %>% 
  pull(Name) %>% 
...
save(fitConfig_t, fitScalar_t, omn_miTest_t, 
     fitAbo_t, anchorTest_t, anchorOut_t, anchors_t,
     file = "3_output/Results/2_MNLFA/MNLFA_fit_t.RData")
...
freeparT[1,c(anchors_t)] <- FALSE

freeparL <- matrix(TRUE, nrow=10, ncol=1, byrow=TRUE)
freeparL[anchors_t,1] <- FALSE
...
modAnchors_t <- mxModel(model="AnchorsOnly", 
                        matT, matT0, matB1, matB2, matB3, matB4, matB5,
                        matL, matL0, matC1, matC2, matC3, matC4, matC5,
                        matE, matE0, matD1, matD2, matD3, matD4, matD5,
                        matP, matP0, matH1, matH2, matH3, matH4, matH5,
...
fitAnchors_t <- mxRun(modAnchors_t)

load("3_output/Results/2_MNLFA/MNLFA_fit_t.RData")

save(fitConfig_t, fitScalar_t, omn_miTest_t, 
     fitAbo_t, anchorTest_t, anchorOut_t, anchors_t, fitAnchors_t,
     file = "3_output/Results/2_MNLFA/MNLFA_fit_t.RData")


### 3.5.4 Run constrained model for each moderator and individual DIF path (except the anchors) ----
testIn_t <- c(1:10)[-c(anchors_t)]
...
apo_grid_t <- expand_grid(
  item = testIn_t, mods = c("age", "edu", "urb", "thr", "dep"), par = c("intercept", "loading")
) 

mxOption(key="Number of Threads", value=8)
future::plan(multisession, workers = 20)

fitApo_t <- 1:nrow(apo_grid_t) |>  
  furrr::future_map_dfr(function(i) {
    
    
    print(glue("{i} - Initiation successful"))
    if(apo_grid_t$par[[i]] == "loading") { 
      freeparTa <- freeparT
      freeparLa <- freeparL
      freeparLa[apo_grid_t$item[[i]],1] <- FALSE
      
      # Constrain parL only for current moderator
      matC1 <- mxMatrix(type="Full", nrow=nv, ncol=1,
                        free=if(apo_grid_t$mods[[i]] == "age") freeparLa else freeparL,
                        values=0,
                        name="matC1")
      matC2 <- mxMatrix(type="Full", nrow=nv, ncol=1,
                        free=if(apo_grid_t$mods[[i]] == "edu") freeparLa else freeparL,
                        values=0,
                        name="matC2")
      matC3 <- mxMatrix(type="Full", nrow=nv, ncol=1,
                        free=if(apo_grid_t$mods[[i]] == "urb") freeparLa else freeparL,
                        values=0,
                        name="matC3")
      matC4 <- mxMatrix(type="Full", nrow=nv, ncol=1,
                        free=if(apo_grid_t$mods[[i]] == "thr") freeparLa else freeparL,
                        values=0,
                        name="matC4")
      matC5 <- mxMatrix(type="Full", nrow=nv, ncol=1,
                        free=if(apo_grid_t$mods[[i]] == "dep") freeparLa else freeparL,
                        values=0,
                        name="matC5")
      matB1 <- matB1_unc
      matB2 <- matB2_unc
    ...
    if(apo_grid_t$par[[i]] == "intercept") { 
      freeparTa <- freeparT
      freeparLa <- freeparL
      freeparTa[apo_grid_t$item[[i]]] <- FALSE
      
      # Constrain parT only for current moderator
      matB1 <- mxMatrix(type="Full", nrow=1, ncol=nv,
                        free=if(apo_grid_t$mods[[i]] == "age") freeparTa else freeparT,
                        values=0,
                        name="matB1")
      matB2 <- mxMatrix(type="Full", nrow=1, ncol=nv,
                        free=if(apo_grid_t$mods[[i]] == "edu") freeparTa else freeparT,
                        values=0,
                        name="matB2")
      matB3 <- mxMatrix(type="Full", nrow=1, ncol=nv,
                        free=if(apo_grid_t$mods[[i]] == "urb") freeparTa else freeparT,
                        values=0,
                        name="matB3")
      matB4 <- mxMatrix(type="Full", nrow=1, ncol=nv,
                        free=if(apo_grid_t$mods[[i]] == "thr") freeparTa else freeparT,
                        values=0,
                        name="matB4")
      matB5 <- mxMatrix(type="Full", nrow=1, ncol=nv,
                        free=if(apo_grid_t$mods[[i]] == "dep") freeparTa else freeparT,
                        values=0,
                        name="matB5")
      matC1 <- matC1_unc
      matC2 <- matC2_unc
    ...
    modApo_t <- mxModel(model=glue("Anchors_plus_{apo_grid_t$item[[i]]}_{apo_grid_t$mods[[i]]}_{apo_grid_t$par[[i]]}"), 
                        matT, matT0, matB1, matB2, matB3, matB4, matB5,
                        matL, matL0, matC1, matC2, matC3, matC4, matC5,
                        matE, matE0, matD1, matD2, matD3, matD4, matD5,
                        matP, matP0, matH1, matH2, matH3, matH4, matH5,
    ...
    tibble(
      item = apo_grid_t$item[[i]],
      mod  = apo_grid_t$mods[[i]],
      par  = apo_grid_t$par[[i]],
      fit  = list(fitApo_t)
    )
...
miTest_Apo_t  <- fitApo_t |> 
  rowwise() |> 
  mutate(
    fitcompare = mxCompare(fitAnchors_t, fit) |> as_tibble() |> list(),
